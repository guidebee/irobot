...
rem current working directory, not this script's location -- cd into
rem tools\ first so the launcher works from anywhere, same pattern as
rem agent_client.cmd.
cd /d "%SCRIPT_DIR%"
where py >nul 2>nul
if %ERRORLEVEL%==0 (
    ...
    rem broken Microsoft Store stub -- see agent_client.cmd
